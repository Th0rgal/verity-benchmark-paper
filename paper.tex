...
complete runs range down to 31 tasks. These results indicate meaningful
progress but also show that proof generation for contract verification remains far from
solved.
\end{abstract}
...
are not merely failing uniformly, but the best result still leaves 69 tasks
unverified. This makes the benchmark useful for measuring incremental progress
rather than for declaring the problem solved.
...
budget profile, or 96\%. This suggests that some apparent model gaps are
budget-sensitive, but the experiment is deliberately reported as a sidecar
because its adaptive cascade methodology is not the same as the complete-suite
leaderboard.
