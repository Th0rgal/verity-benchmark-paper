...
remain expensive to write. Recent AI agents can edit code, invoke tools, and
repair compiler errors, which makes proof generation a concrete target rather
than a purely manual activity. Existing benchmarks, however, rarely ask agents
to produce complete, machine-checkable proofs for contract obligations instead
of informal arguments or unit tests. \toolname{} is an open evaluation suite
for measuring whether AI agents can generate \lean{} proofs of Ethereum smart
...
design, its v0.1 composition, and preliminary release results across all
published complete-suite runs. The strongest complete run, GPT-5.5, verifies
66 of 135 tasks; GLM 5.2 verifies 53, Opus 4.8 verifies 45, and the remaining
complete runs range down to 31 tasks. The strongest complete run solves less
than half of the suite, while a budget-scaling sidecar shows that some failures
are sensitive to effort rather than only model choice.
\end{abstract}
...
inspect a workspace, call a compiler, read error messages, and modify proof scripts. In
...
\toolname{} targets this missing evaluation setting. It evaluates agents on proof-only
...
ownership structures. Third, it reports the complete-suite and partial-run
results published with v0.1. The best complete run solves 49\% of tasks.
...
interfaces, sampling controls, rate limits, and agent wrappers differ. The
v0.1 release separates complete-suite runs from partial artifacts, so the
...
grok-build-0.1 verifies 41, MiniMax M3 verifies 38, Kimi K2.7 verifies 31, and
no complete run crosses the halfway point. The spread matters: current agents
do not fail uniformly, but the best result still leaves 69 tasks unverified.
...
same v0.1 release, including partial and invalid runs. The most notable partial
artifact is GPT-5.5 Pro, which verifies 35 of 54 attempted tasks. That is a
high within-run rate, but it is not a complete-suite result and therefore
...
It covers accounting, storage, access control, and solvency
...
complete run verifies 66 of 135 tasks, and several classes of protocol
reasoning remain mostly unsolved.
